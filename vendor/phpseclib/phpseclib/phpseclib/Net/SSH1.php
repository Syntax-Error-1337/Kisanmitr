    ...
    */
    ...
    function __construct($host, $port = 22, $timeout = 10, $cipher = self::CIPHER_3DES)
    {
        $this->protocol_flags = array(
            ...
            3  => 'NET_SSH1_G_SESSION_KEY',
            4  => 'NET_SSH1_G_USER',
            9  => 'NET_SSH1_G_AUTH_PASSWORD',
            10 => 'NET_SSH1_G_REQUEST_PTY',
            12 => 'NET_SSH1_G_EXEC_SHELL',
            13 => 'NET_SSH1_G_EXEC_CMD',
            14 => 'NET_SSH1_SMSG_SUCCESS',
            15 => 'NET_SSH1_SMSG_FAILURE',
            16 => 'NET_SSH1_G_STDIN_DATA',
            17 => 'NET_SSH1_SMSG_STDOUT_DATA',
            18 => 'NET_SSH1_SMSG_STDERR_DATA',
            19 => 'NET_SSH1_G_EOF',
            20 => 'NET_SSH1_SMSG_EXITSTATUS',
            33 => 'NET_SSH1_G_EXIT_CONFIRMATION'
        );
    ...
    function _connect()
    {
        ...
        $data = pack('C2a*na*N', NET_SSH1_G_SESSION_KEY, $cipher, $anti_spoofing_cookie, 8 * strlen($double_encrypted_session_key), $double_encrypted_session_key, 0);

        if (!$this->_send_binary_packet($data)) {
            user_error('Error sending SSH_G_SESSION_KEY');
            return false;
        }
    ...
    function login($username, $password = '')
    {
        ...
        $data = pack('CNa*', NET_SSH1_G_USER, strlen($username), $username);

        if (!$this->_send_binary_packet($data)) {
            user_error('Error sending SSH_G_USER');
            return false;
        }
        ...
        $data = pack('CNa*', NET_SSH1_G_AUTH_PASSWORD, strlen($password), $password);

        if (!$this->_send_binary_packet($data)) {
            user_error('Error sending SSH_G_AUTH_PASSWORD');
            return false;
        }
    ...
    function exec($cmd, $block = true)
    {
        ...
        $data = pack('CNa*', NET_SSH1_G_EXEC_CMD, strlen($cmd), $cmd);

        if (!$this->_send_binary_packet($data)) {
            user_error('Error sending SSH_G_EXEC_CMD');
            return false;
        }
        ...
        $data = pack('C', NET_SSH1_G_EXIT_CONFIRMATION);
    ...
    function _initShell()
    {
        ...
        $data = pack('CNa*N4C', NET_SSH1_G_REQUEST_PTY, strlen('vt100'), 'vt100', 24, 80, 0, 0, self::TTY_OP_END);

        if (!$this->_send_binary_packet($data)) {
            user_error('Error sending SSH_G_REQUEST_PTY');
            return false;
        }
        ...
        $data = pack('C', NET_SSH1_G_EXEC_SHELL);

        if (!$this->_send_binary_packet($data)) {
            user_error('Error sending SSH_G_EXEC_SHELL');
            return false;
        }
    ...
    function interactiveWrite($cmd)
    {
        ...
        $data = pack('CNa*', NET_SSH1_G_STDIN_DATA, strlen($cmd), $cmd);

        if (!$this->_send_binary_packet($data)) {
            user_error('Error sending SSH_G_STDIN');
            return false;
        }
